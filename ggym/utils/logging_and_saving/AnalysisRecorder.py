...
class ScreenShotter:
    def __init__(self, env, experiment_name, log_dir):
        ...
        self.folderpath = os.path.join(
            LEGGED_GYM_ROOT_DIR, "logs", self.experiment_name, "analysis"
        )
    ...
    def screenshot(self, image):
        ...
        filepath = os.path.join(
            self.folderpath, f"{self.run_name}_{self.screenshot_cnt}.pdf"
        )
...
class AnalysisRecorder:
    def __init__(self, env, experiment_name, log_dir):
        ...
        self.run_name = os.path.basename(log_dir)
        self.frames = []
        self.states_dict = defaultdict(
            list
        )  # defaultdict(lambda: defaultdict(list)), defaultdict(list)
    ...
    def log(self, image, states_dict, commands_dict):
        ...
        for key, value in states_dict.items():
            if key == "root_states":
                ...
                self.states_dict["rf_contact_forces_x"].append(
                    value[self.env.feet_ids[0], 0].item()
                )
                self.states_dict["rf_contact_forces_y"].append(
                    value[self.env.feet_ids[0], 1].item()
                )
                self.states_dict["rf_contact_forces_z"].append(
                    value[self.env.feet_ids[0], 2].item()
                )
                self.states_dict["lf_contact_forces_x"].append(
                    value[self.env.feet_ids[1], 0].item()
                )
                self.states_dict["lf_contact_forces_y"].append(
                    value[self.env.feet_ids[1], 1].item()
                )
                self.states_dict["lf_contact_forces_z"].append(
                    value[self.env.feet_ids[1], 2].item()
                )
    ...
    def make_animation(self):
        ...
        ax.set_ylim(
            min(min(COM_vel_x), min(COM_vel_y), min(COM_dvel_x), min(COM_dvel_y)) - 0.1,
            max(max(COM_vel_x), max(COM_vel_y), max(COM_dvel_x), max(COM_dvel_y)) + 0.1,
        )
        ...
        bx.set_ylim(
            min(min(step_length), min(step_width), min(dstep_length), min(dstep_width))
            - 0.1,
            max(max(step_length), max(step_width), max(dstep_length), max(dstep_width))
            + 0.1,
        )
    ...
    def make_contact_forces_animation(self):
        ...
        def rf_contact_forces_2D_init():
            rf_contact_forces_x_ani.set_data(
                np.linspace(0, 1, 0), rf_contact_forces_x[0:0]
            )
            rf_contact_forces_y_ani.set_data(
                np.linspace(0, 1, 0), rf_contact_forces_y[0:0]
            )
            rf_contact_forces_z_ani.set_data(
                np.linspace(0, 1, 0), rf_contact_forces_z[0:0]
            )
            return [
                rf_contact_forces_x_ani,
                rf_contact_forces_y_ani,
                rf_contact_forces_z_ani,
                total_weight_line,
            ]

        def rf_contact_forces_2D_update(i):
            rf_contact_forces_x_ani.set_data(
                np.linspace(0, i - 1, i), rf_contact_forces_x[0:i]
            )
            rf_contact_forces_y_ani.set_data(
                np.linspace(0, i - 1, i), rf_contact_forces_y[0:i]
            )
            rf_contact_forces_z_ani.set_data(
                np.linspace(0, i - 1, i), rf_contact_forces_z[0:i]
            )
            return [
                rf_contact_forces_x_ani,
                rf_contact_forces_y_ani,
        ...
        (rf_contact_forces_x_ani,) = ax.plot(
            [], [], color="r", label="right foot contact forces x"
        )
        (rf_contact_forces_y_ani,) = ax.plot(
            [], [], color="g", label="right foot contact forces y"
        )
        (rf_contact_forces_z_ani,) = ax.plot(
            [], [], color="b", label="right foot contact forces z"
        )
        total_weight_line = ax.axhline(
            y=self.env.total_weight, color="k", linestyle="--", label="total weight"
        )
        ...
        def lf_contact_forces_2D_init():
            lf_contact_forces_x_ani.set_data(
                np.linspace(0, 1, 0), lf_contact_forces_x[0:0]
            )
            lf_contact_forces_y_ani.set_data(
                np.linspace(0, 1, 0), lf_contact_forces_y[0:0]
            )
            lf_contact_forces_z_ani.set_data(
                np.linspace(0, 1, 0), lf_contact_forces_z[0:0]
            )
            return [
                lf_contact_forces_x_ani,
                lf_contact_forces_y_ani,
                lf_contact_forces_z_ani,
                total_weight_line,
            ]

        def lf_contact_forces_2D_update(i):
            lf_contact_forces_x_ani.set_data(
                np.linspace(0, i - 1, i), lf_contact_forces_x[0:i]
            )
            lf_contact_forces_y_ani.set_data(
                np.linspace(0, i - 1, i), lf_contact_forces_y[0:i]
            )
            lf_contact_forces_z_ani.set_data(
                np.linspace(0, i - 1, i), lf_contact_forces_z[0:i]
            )
            return [
                lf_contact_forces_x_ani,
                lf_contact_forces_y_ani,
        ...
        (lf_contact_forces_x_ani,) = bx.plot(
            [], [], color="r", label="left foot contact forces x"
        )
        (lf_contact_forces_y_ani,) = bx.plot(
            [], [], color="g", label="left foot contact forces y"
        )
        (lf_contact_forces_z_ani,) = bx.plot(
            [], [], color="b", label="left foot contact forces z"
        )
        total_weight_line = bx.axhline(
            y=self.env.total_weight, color="k", linestyle="--", label="total weight"
        )
        bx.legend(loc="upper right")
        ...
        anim = FuncAnimation(
            ...
            init_func=_init_func,
            func=_update_func,
            frames=range(self.episode_length),
            interval=50,
            blit=False,
        )
...
class CSVLogger:
    ...
    def __init__(self, env, experiment_name, log_dir, max_it):
        ...
        self.info_array = np.zeros(
            (max_it, 1 + 13 + 20)
        )  # Time, root_states, joint pos/vel
        self.episode_length = 0
        self.headers = [
            "ts",
            "bp_x",
    ...
    def log(self):
        ...
        # Log root_states
        self.info_array[self.episode_length, 1:14] = (
            self.env.root_states[0].cpu().numpy()
        )

        # Re-shuffle [x,y,z,w] to [w,x,y,z]
        self.info_array[self.episode_length, 4] = (
            self.env.root_states[0, 6].cpu().numpy()
        )
        self.info_array[self.episode_length, 5:8] = (
            self.env.root_states[0, 3:6].cpu().numpy()
        )
    ...
    def save_csv(self, folderpath):
        ...
        np.savetxt(
            filepath,
            self.info_array[: self.episode_length, :],
            delimiter=",",
...
class DictLogger:
    ...
    def log(self):
        for key in self.keys:
            self.data_dict[key][self.episode_length] = (
                getattr(self.env, key)[self.env_idx].cpu().numpy()
            )
...
class SuccessRater:
    ...
    def save_and_exit(self):
        ...
        for key in self.keys:
            self.data_dict[key]["success_rate"] = (
                self.data_dict[key]["timed_out_cnt"] / self.data_dict[key]["reset_cnt"]
            )
