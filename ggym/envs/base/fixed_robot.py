...
from ggym.envs.base.base_task import BaseTask
...
class FixedRobot(BaseTask):
    def __init__(
        self, cfg: FixedRobotCfg, sim_params, physics_engine, sim_device, headless
    ):
    ...
    def step(self, actions):
        ...
        for _ in range(self.cfg.control.decimation):
            ...
            if self.cfg.control.exp_avg_decay:
                self.action_avg = exp_avg_filter(
                    self.actions, self.action_avg, self.cfg.control.exp_avg_decay
                )
                self.torques = self._compute_torques(self.action_avg).view(
                    self.torques.shape
                )
            else:
            ...
            torques_to_gym_tensor = torch.zeros(
                size=(self.num_envs, self.num_dof), device=self.device
            )
            next_torques_idx = 0
    ...
    def compute_observations(self):
        ...
        self.ctrl_hist[:, :nact] = (
            self.actions * self.cfg.control.action_scale + self.default_act_pos
        )

        dof_pos = (self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos

        self.obs_buf = torch.cat(
            (dof_pos, self.dof_vel * self.obs_scales.dof_vel, self.ctrl_hist), dim=-1
        )
    ...
    def _get_noise_scale_vec(self):
        ...
        noise_vec = torch.zeros_like(self.obs_buf[0], device=self.device)
        ...
        if self.cfg.domain_rand.randomize_friction:
            if env_id == 0:
                ...
                bucket_ids = torch.randint(
                    0, num_buckets, (self.num_envs, 1), device=self.device
                )
    ...
    def _reward_action_rate(self):
        ...
        error = (
            torch.square(self.ctrl_hist[:, :nact] - self.ctrl_hist[:, nact : 2 * nact])
            / dt2
        )
